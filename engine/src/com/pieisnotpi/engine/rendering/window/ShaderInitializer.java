...
public interface ShaderInitializer
{
    ...
    {
        inst.shaders.put(ADSShader.ID, new ADSShader(inst.window).init());
        inst.shaders.put(ColorShader.ID, new ColorShader(inst.window).init());
        inst.shaders.put(TextShader.ID, new TextShader(inst.window).init());
        inst.shaders.put(TexCShader.ID, new TexCShader(inst.window).init());
        inst.shaders.put(TexShader.ID, new TexShader(inst.window).init());
    }
}
